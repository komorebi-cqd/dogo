<template>
  <div class="head">
    ...
    <ul class="head-right">
      <li>
        <a @click="goHome('')">{{ $t("head.nav1") }}</a>
      </li>
      <li>
        <a @click="goHome('#type')">{{ $t("head.nav2") }}</a>
      </li>
      <li>
        <a @click="goHome('#prop')">{{ $t("head.nav3") }}</a>
      </li>
      <li>
        <a @click="goHome('#earn')">{{ $t("head.nav4") }}</a>
      </li>
      <li>
        <a @click="goHome('#token')">{{ $t("head.nav5") }}</a>
      </li>
      <li>
        <a @click="goHome('#contact')">{{ $t("head.nav6") }}</a>
      </li>
      <li v-if="tokens">
        <el-popover width="300px" placement="bottom" trigger="hover">
          ...
          <p class="token">
            ...
            <el-button size="small" :loading="usdLoading" @click="onUsdTransfer" type="primary">{{$t('head.buySuperShield')}} </el-button>
...
import { inviteList, withdraw, exchangeShield, dtTransfer, usdTransfer, getUsdAddress, mintTransfer } from "@/api";
import moment from "moment";
import { ElNotification } from "element-plus";
import i18n from "@/language/i18n";
const getText = key => {
  return i18n.global.messages[i18n.global.locale][key];
};
